...
# ═══════════════════════════════════════════════════════════════════════════
#  CONFIGURATION — Edit these values to change evaluation parameters
# ═══════════════════════════════════════════════════════════════════════════
CONFIG = {
    ...
    # Don't apply isotonic calibrator even if it's bundled with the checkpoint
...
# ═══════════════════════════════════════════════════════════════════════════
def main():
    # Resolve device (auto-detect if not explicitly set)
    ...
    print(f"  Val file    : {CONFIG['VAL_JSONL']}")
    ...
    # ── 1. Load val data ──────────────────────────────────────────────────
    if not os.path.exists(CONFIG["VAL_JSONL"]):
        sys.exit(f"ERROR: val.jsonl not found: {CONFIG['VAL_JSONL']}")

    print("\n[1/3] Loading val.jsonl …")
    records = load_jsonl(CONFIG["VAL_JSONL"])
    ...
    pred_p_ai = detector.predict_p_ai(texts, batch_size=CONFIG["BATCH_SIZE"])
    ...
    if not CONFIG["SKIP_CALIBRATION"]:
        calibrator = try_load_calibrator(CONFIG["MODEL_PATH"])
