"""
ADAL v5 — Honest Generalization Tests (RoBERTa-large)
======================================================
Base    : "RADAR: Robust AI-Text Detection via Adversarial Learning"
          Hu et al., NeurIPS 2023 — https://arxiv.org/pdf/2307.03838
Datasets: RAID + RAID-extra (Code/Czech/German)  AND  PAN CLEF 2026 JSONL
          Switch via --dataset {raid|panclef} CLI flag.

v5 extensions over v4 (per-generator weakness mitigation):
  • RAID + RAID-extra training — adds Code, Czech News, German News domains.
    The detector has never seen these in v4 — biggest data gap to close.
  • Held-out generators in val — train on 8 generators, val on 3 unseen.
    Measures TRUE cross-generator generalization (not memorization).
  • Adversarial val split — val now includes RAID's REAL adversarial attacks
    (homoglyph, paraphrase, etc.), not just synthetic-attack augmentation.
  • Per-domain reporting — separate AUROC/TPR for prose vs Code vs Czech etc.

The honest result that v5 produces is most likely:
  - High AUROC on training generators (memorisation)
  - Substantially lower AUROC on held-out generators (real generalization)
  - Lower AUROC on Code/Czech/German if RoBERTa wasn't pretrained for them
This is exactly what TACL/ACL reviewers want to see — a clear-eyed measurement
of where the detector breaks down.
"""
...
# ADAL v4: RAID is optional. PAN CLEF runs don't need it. We try to import,
# and fall back gracefully if raid-bench is not installed.
try:
    from raid.utils import load_data as raid_load_data
...
# ══════════════════════════════════════════════════════════════════════════════
#  DATASET SELECTOR — ADAL v4
# ══════════════════════════════════════════════════════════════════════════════
# DATASET = "raid" or "panclef". Override with --dataset CLI flag.
#
# raid    → loads via raid.utils (requires `pip install raid-bench`).
#           Uses NUM_HUMAN_SAMPLES / SAMPLES_PER_GENERATOR caps + 90/10 split.
# panclef → loads from JSONL files at TRAIN_JSONL / VAL_JSONL.
#           Uses pre-defined train/val files. Caps default to None (use all).
DATASET = "raid"

# RAID dataset (used only when DATASET="raid")
...
# ══════════════════════════════════════════════════════════════════════════════
#  ADAL v5 — Honest Generalization Setup
# ══════════════════════════════════════════════════════════════════════════════
# v4 results plateaued at AUROC=0.9954 / TPR@1%FPR=0.9473 because the val split
# came from the SAME distribution as train (90/10 of attack="none"). v5 adds
# three orthogonal generalization tests to expose where the detector actually
# fails — the kind of result a TACL/ACL paper needs.
#
#   1. RAID + RAID-extra: train across more domains (Code, Czech, German)
#   2. Held-out generators in val: cross-generator generalization
#   3. Adversarial val split: per-attack robustness reported during training
# ══════════════════════════════════════════════════════════════════════════════

# ── 1. Include RAID-extra dataset ────────────────────────────────────────────
# RAID-extra adds 3 new domains: Python Code, Czech News, German News.
# The detector has never seen these in v4 — this is the biggest data gap.
# Concatenated with RAID train; same 11 generators, same schema.
USE_RAID_EXTRA = True

# ── 2. Held-out generators (cross-generator generalization test) ─────────────
# Generators in this list are EXCLUDED from training and used ONLY for val.
# This measures whether the detector generalizes to unseen generators or
# merely memorizes the training distribution. Set to [] to disable.
#
# Recommended: held out one strong (gpt-4), one already-weak (cohere-chat),
# one cross-family (mpt-chat). Picks span GPT/Cohere/MPT families.
HELDOUT_GENERATORS: List[str] = ["gpt4", "cohere-chat", "mpt-chat"]

# ── 3. Adversarial validation split ──────────────────────────────────────────
# v4 trained AND validated on attack="none". Real RAID benchmark uses
# adversarial attacks. v5 adds adversarial AI rows to the val split so we
# get per-attack AUROC during training (not just post-hoc).
# Train remains attack="none" — the deterministic attack pool already
# generates training-time adversarial examples.
USE_ADVERSARIAL_VAL = True

# Cap on adversarial AI val rows per (generator, attack) — prevents val
# from ballooning to millions of rows. None = no cap.
ADVERSARIAL_VAL_PER_ATTACK_CAP = 100

# ── Per-domain reporting ─────────────────────────────────────────────────────
# When USE_RAID_EXTRA=True or you train on multiple domains, log val AUROC
# split by domain so you can see whether code/Czech/German are tanking the
# average. Adds one extra log table per validation cycle.
REPORT_PER_DOMAIN_METRICS = True

# PAN CLEF JSONL paths (used only when DATASET="panclef")
# Each line: { "text": ..., "label": 0|1, "model": ..., "id": ..., "genre": ... }
...
# Set None to use all available data. For PAN CLEF, None means use all.
NUM_HUMAN_SAMPLES     = None  # RAID default; set None for full PAN CLEF
SAMPLES_PER_GENERATOR = None   # RAID default; set None for full PAN CLEF

# Train/val split ratio — applied AFTER sampling the caps above.
# Used only by RAID loader; PAN CLEF uses pre-split JSONL files.
VAL_SPLIT_RATIO = 0.1

# ── Multi-generator sampling strategy ────────────────────────────────────────
# "uniform"           | sample one generator per step uniformly at random
# "round_robin"       | cycle through generators
# "mixed"             | use ALL generators each step (default for ADAL v3)
# "curriculum"        | start with easiest, escalate as detector wins
# "hard_negative"     | ADAL v4 NEW: dynamically up-weight worst generators
#                      based on running per-generator val TPR@1%FPR
GENERATOR_SAMPLING_STRATEGY = "hard_negative"
...
# ══════════════════════════════════════════════════════════════════════════════
#  ADAL v4 — Per-Generator Weakness Mitigation
# ══════════════════════════════════════════════════════════════════════════════
# v3's bidirectional game improved global TPR@1%FPR but left per-generator gaps:
#   - mpt: 0.808, cohere: 0.883, cohere-chat: 0.891 (RAID v3 best ckpt)
# v4 adds three orthogonal techniques to close those gaps, all data-side
# (no backbone change — RoBERTa-large stays):
#
#   1. Hard-negative sampling (Pangram-style active learning, COLING 2025)
#   2. Multiclass auxiliary head (Leidos COLING 2025 winning technique)
#   3. Worst-generator early stopping (max(min(per-gen TPR@1%FPR)))
# ══════════════════════════════════════════════════════════════════════════════

# ── 1. Hard-negative generator sampling ──────────────────────────────────────
# When GENERATOR_SAMPLING_STRATEGY="hard_negative", weight per-generator sampling
# probability inversely to recent val TPR@1%FPR. Worst generators get more
# training samples per batch.
#   weight_g = (1 - tpr_g) ** HARD_NEG_WEIGHT_POWER + HARD_NEG_BASE_WEIGHT
# - power=2 → quadratic emphasis on worst generators (bigger gap = much more samples)
# - base=0.1 → guarantee even strong generators get sampled occasionally (avoid forgetting)
HARD_NEG_WEIGHT_POWER       = 2.0
HARD_NEG_BASE_WEIGHT        = 0.1
# Update the hard-negative weights every N validation cycles.
HARD_NEG_UPDATE_EVERY_VALS  = 1
# How many warmup steps to use uniform sampling before switching to hard-neg.
# (Need at least 1 validation cycle to know per-generator scores.)
HARD_NEG_WARMUP_STEPS       = 5

# ── 2. Multiclass auxiliary generator-attribution head ───────────────────────
# Adds a second classification head to the detector that predicts WHICH
# generator produced an AI text (or "human" for human texts). Multi-task
# learning forces the encoder to learn generator-discriminative features,
# which transfers back to improved binary detection — especially on confused
# generators where the auxiliary head's gradient signal is strongest.
AUX_HEAD_ENABLED            = True
AUX_HEAD_LOSS_WEIGHT        = 0.3   # weight of auxiliary loss in total loss
                                    # 0.3 = mild — main task still dominates
AUX_HEAD_DROPOUT            = 0.1
# AUX_HEAD_NUM_CLASSES is set automatically at runtime from the loaded data
# (= 1 + number of generators, +1 for "human" class).

# ── 3. Worst-generator early stopping ────────────────────────────────────────
# Instead of selecting the best checkpoint by GLOBAL TPR@1%FPR (pooled across
# all generators), use the WORST-PERFORMING GENERATOR's TPR@1%FPR. This forces
# the optimizer to focus on closing the gap rather than improving easy generators.
# When True, "best metric" = min(per-generator TPR@1%FPR).
USE_WORST_GENERATOR_FOR_BEST = True
...
HUMANIZER_PROMPT_PREFIX     = "Rewrite in a formal, polished, AI-like style: "
...
HF_TOKEN              = "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"        # ← paste your HF write token
...
HF_DETECTOR_REPO      = "adal_v5_panclef"     # ← repo name for the detector
HF_PARAPHRASER_REPO   = "adal_v5_panclef-paraphraser"       # ← repo name for the paraphraser
...
def _parse_raid_df(
    ...
    return_metadata: bool = False,           # ADAL v5: also return domain/attack
    ...
    ADAL v4 fix: deduplicate human texts by `source_id` (RAID schema). Each
    unique human document is duplicated as a row for every (model, decoding)
    combination — naive iteration would duplicate humans 11×4=44 times. We
    use source_id to keep only ONE row per unique human document.

    ADAL v5: when return_metadata=True, also returns parallel dicts of
    (domain, attack) tuples — used for per-domain val metrics + adversarial
    val splits.
    """
    ...
    human_meta:  List[Tuple[str, str]] = []   # (domain, attack) per human
    ai_pool:     Dict[str, List[str]] = defaultdict(list)
    ai_meta:     Dict[str, List[Tuple[str, str]]] = defaultdict(list)
    seen_human_source_ids: set        = set()  # ADAL v4: dedup humans by source_id
    seen_human_texts:      set        = set()  # backup dedup by text content
    ...
    for _, row in df.iterrows():
        ...
        if model == "human":
            # ADAL v4: dedup by source_id first (preferred), fallback to text
            if source_id and source_id in seen_human_source_ids:
                continue
            if not source_id and text in seen_human_texts:
                continue
            seen_human_source_ids.add(source_id) if source_id else None
            seen_human_texts.add(text)
            if n_human is None or len(human_texts) < n_human:
                human_texts.append(text)
                human_meta.append((domain, attack))
        else:
            ...
            if n_ai_per_gen is None or len(ai_pool[model]) < n_ai_per_gen:
                ...
                ai_meta[model].append((domain, attack))

    # Shuffle while keeping (text, metadata) pairs aligned
    if human_texts:
        combined = list(zip(human_texts, human_meta))
        random.shuffle(combined)
        human_texts, human_meta = [list(t) for t in zip(*combined)]
    for m in ai_pool:
        if ai_pool[m]:
            combined = list(zip(ai_pool[m], ai_meta[m]))
            random.shuffle(combined)
            ai_pool[m], ai_meta[m] = [list(t) for t in zip(*combined)]

    logger.info(f"  Human texts (deduped) : {len(human_texts):,}")
    ...
    if return_metadata:
        return human_texts, dict(ai_pool), human_meta, dict(ai_meta)
...
def load_raid_multigenerator() -> Tuple[
    ...
    """
    Load RAID train (and optionally RAID-extra), with v5 generalization tests.

    v5 strategy (in order of application):
      1. Load RAID train + optionally RAID-extra (Code, Czech, German domains).
         BOTH splits with include_adversarial=USE_ADVERSARIAL_VAL — adversarial
         rows go ONLY to val, never to train.
      2. Deduplicate humans by source_id.
      3. Build train (clean only, attack="none") and val (clean + optional
         adversarial AI) splits per generator.
      4. HELDOUT_GENERATORS are EXCLUDED from train and used ONLY for val.
         Their full data (clean + adversarial) becomes part of val.
      5. Class-balance training AI to ~match human count.

    Two new module-level globals are populated:
      _v5_val_human_meta  : List[(domain, attack)] aligned with returned human_val
      _v5_val_ai_meta     : Dict[gen, List[(domain, attack)]] aligned with ai_val

    These are used by the validation loop for per-domain / per-attack reporting.
    """
    global _v5_val_human_meta, _v5_val_ai_meta, _v5_heldout_in_val

    if not _RAID_AVAILABLE:
        raise RuntimeError(
            "DATASET='raid' but raid-bench is not installed.\n"
            "Install with: pip install raid-bench"
        )

    logger.info("Loading RAID dataset via raid.utils.load_data …")
    logger.info(f"  USE_RAID_EXTRA          = {USE_RAID_EXTRA}")
    logger.info(f"  HELDOUT_GENERATORS      = {HELDOUT_GENERATORS}")
    logger.info(f"  USE_ADVERSARIAL_VAL     = {USE_ADVERSARIAL_VAL}")
    logger.info(f"  Caps: human={NUM_HUMAN_SAMPLES}  AI={SAMPLES_PER_GENERATOR}/gen  "
                f"val_ratio={VAL_SPLIT_RATIO}")

    # ── Stage 1: load RAID train (and optionally RAID-extra) ─────────────
    # If we want adversarial val data, we need include_adversarial=True so
    # adversarial rows are in the dataframe. We split clean vs adversarial
    # afterwards via the "attack" column.
    include_adv = USE_ADVERSARIAL_VAL

    train_df = raid_load_data(split="train", include_adversarial=include_adv)
    logger.info(f"  RAID train rows (include_adv={include_adv}): {len(train_df):,}")

    if USE_RAID_EXTRA:
        try:
            extra_df = raid_load_data(split="extra", include_adversarial=include_adv)
            logger.info(f"  RAID extra rows (include_adv={include_adv}): {len(extra_df):,}")
            import pandas as pd
            full_df = pd.concat([train_df, extra_df], ignore_index=True)
            logger.info(f"  Combined train+extra: {len(full_df):,} rows")
            del train_df, extra_df
        except Exception as e:
            logger.warning(f"  Could not load RAID-extra: {e}")
            logger.warning(f"  Falling back to RAID train only.")
            full_df = train_df
    else:
        full_df = train_df

    # ── Stage 2: parse the FULL dataframe (clean + adversarial together) ──
    # We pass attack_filter=None to keep BOTH clean and adversarial rows.
    # We'll separate them downstream by inspecting the per-row metadata.
    human_all, ai_all_by_model, human_meta, ai_meta_by_model = _parse_raid_df(
        full_df, "full", NUM_HUMAN_SAMPLES, n_ai_per_gen=None,
        attack_filter=None,           # accept all rows; we split by metadata
        return_metadata=True,
    )
    del full_df  # free memory before downstream processing

    # ── Stage 3: separate clean from adversarial AI per generator ────────
    # Humans don't get attacked in RAID — every human row has attack="none".
    # AI rows split into clean (attack="none") for train AND val, and
    # adversarial (attack!="none") for val only.
    ai_clean_by_model:  Dict[str, List[str]]              = {}
    ai_clean_meta:      Dict[str, List[Tuple[str, str]]]  = {}
    ai_adv_by_model:    Dict[str, List[str]]              = defaultdict(list)
    ai_adv_meta:        Dict[str, List[Tuple[str, str]]]  = defaultdict(list)

    for gen, texts in ai_all_by_model.items():
        clean_t, clean_m = [], []
        for t, m in zip(texts, ai_meta_by_model[gen]):
            if m[1] == "none":
                clean_t.append(t)
                clean_m.append(m)
            else:
                ai_adv_by_model[gen].append(t)
                ai_adv_meta[gen].append(m)
        ai_clean_by_model[gen] = clean_t
        ai_clean_meta[gen]     = clean_m

    logger.info(
        f"  Clean AI: {sum(len(v) for v in ai_clean_by_model.values()):,}  "
        f"Adversarial AI: {sum(len(v) for v in ai_adv_by_model.values()):,}"
    )

    # ── Stage 4: separate held-out generators from training generators ───
    train_generators = [g for g in ai_clean_by_model if g not in HELDOUT_GENERATORS]
    held_out_in_data = [g for g in HELDOUT_GENERATORS if g in ai_clean_by_model]
    _v5_heldout_in_val = list(held_out_in_data)  # store for banner

    if held_out_in_data:
        logger.info(
            f"  ⚠ HELD-OUT generators (val only): {held_out_in_data}"
        )
        logger.info(
            f"  Training generators ({len(train_generators)}): {sorted(train_generators)}"
        )
    else:
        if HELDOUT_GENERATORS:
            logger.warning(
                f"  HELDOUT_GENERATORS={HELDOUT_GENERATORS} but none found in data!"
            )

    # ── Stage 5: class-balanced quota for training AI ────────────────────
    n_humans   = len(human_all)
    n_train_g  = max(len(train_generators), 1)
    balanced_quota = max(1, n_humans // n_train_g)
    if SAMPLES_PER_GENERATOR is not None:
        per_gen_quota = min(balanced_quota, SAMPLES_PER_GENERATOR)
        logger.info(
            f"  Class-balanced quota: min({balanced_quota}, "
            f"SAMPLES_PER_GENERATOR={SAMPLES_PER_GENERATOR}) = {per_gen_quota}/gen"
        )
    else:
        per_gen_quota = balanced_quota
        logger.info(
            f"  Class-balanced quota: {n_humans} humans / {n_train_g} "
            f"training generators = {per_gen_quota} AI texts/gen"
        )

    # ── Stage 6: 90/10 train/val split for training generators ───────────
    human_train, human_val = _split_list(human_all, VAL_SPLIT_RATIO)
    # Track human metadata for the val portion
    n_human_val   = len(human_val)
    # _split_list shuffles with a fixed seed before splitting; reproduce that
    # split for the metadata to keep alignment.
    rng = random.Random(42)
    indices = list(range(len(human_all)))
    rng.shuffle(indices)
    n_val = max(1, int(len(human_all) * VAL_SPLIT_RATIO))
    val_indices = indices[:n_val]
    human_val_meta = [human_meta[i] for i in val_indices]

    ai_train_by_model: Dict[str, List[str]] = {}
    ai_val_by_model:   Dict[str, List[str]] = {}
    ai_val_meta:       Dict[str, List[Tuple[str, str]]] = {}

    # Training generators: balanced quota, 90/10 split for clean AI
    for gen in train_generators:
        clean_texts = ai_clean_by_model[gen][:per_gen_quota]
        clean_meta_g = ai_clean_meta[gen][:per_gen_quota]
        rng_g = random.Random(42)
        idx_g = list(range(len(clean_texts)))
        rng_g.shuffle(idx_g)
        n_val_g = max(1, int(len(clean_texts) * VAL_SPLIT_RATIO))
        val_idx_g, train_idx_g = idx_g[:n_val_g], idx_g[n_val_g:]

        ai_train_by_model[gen] = [clean_texts[i] for i in train_idx_g]
        ai_val_by_model[gen]   = [clean_texts[i] for i in val_idx_g]
        ai_val_meta[gen]       = [clean_meta_g[i] for i in val_idx_g]

        # Append adversarial val rows for this generator (capped per-attack)
        if USE_ADVERSARIAL_VAL and ai_adv_by_model.get(gen):
            adv_t   = ai_adv_by_model[gen]
            adv_m   = ai_adv_meta[gen]
            if ADVERSARIAL_VAL_PER_ATTACK_CAP:
                # Group by attack, cap each
                by_attack: Dict[str, List[Tuple[str, str, str]]] = defaultdict(list)
                for t, m in zip(adv_t, adv_m):
                    by_attack[m[1]].append((t, m[0], m[1]))
                capped_t, capped_m = [], []
                for atk, items in by_attack.items():
                    for t, dom, atk_ in items[:ADVERSARIAL_VAL_PER_ATTACK_CAP]:
                        capped_t.append(t)
                        capped_m.append((dom, atk_))
                adv_t, adv_m = capped_t, capped_m
            ai_val_by_model[gen].extend(adv_t)
            ai_val_meta[gen].extend(adv_m)

    # Held-out generators: ALL their data goes to val (clean + adversarial)
    for gen in held_out_in_data:
        held_clean = ai_clean_by_model[gen]
        held_meta  = ai_clean_meta[gen]
        # Cap clean held-out to balanced_quota to keep val sizes manageable
        cap = min(len(held_clean), balanced_quota)
        ai_val_by_model[gen] = held_clean[:cap]
        ai_val_meta[gen]     = held_meta[:cap]
    ...
    # Save val metadata to module globals for the validation loop
    _v5_val_human_meta = human_val_meta
    _v5_val_ai_meta    = ai_val_meta

    # ── Final summary ────────────────────────────────────────────────────
    total_train_ai = sum(len(v) for v in ai_train_by_model.values())
    total_val_ai   = sum(len(v) for v in ai_val_by_model.values())
    logger.info(
        f"  Train → human={len(human_train):,}  AI={total_train_ai:,}  "
        f"({len(ai_train_by_model)} generators)"
    )
    logger.info(
        f"  Val   → human={len(human_val):,}  AI={total_val_ai:,}  "
        f"({len(ai_val_by_model)} generators)"
    )
    if held_out_in_data:
        for gen in held_out_in_data:
            logger.info(
                f"    HELD-OUT {gen:20s} → val n={len(ai_val_by_model.get(gen, [])):,}"
            )
    if USE_ADVERSARIAL_VAL:
        # Report per-attack counts in val
        atk_counts: Dict[str, int] = defaultdict(int)
        for gen, metas in ai_val_meta.items():
            for dom, atk in metas:
                atk_counts[atk] += 1
        logger.info(f"  Val attack distribution: {dict(atk_counts)}")
    if REPORT_PER_DOMAIN_METRICS:
        dom_counts: Dict[str, int] = defaultdict(int)
        for dom, atk in human_val_meta:
            dom_counts[dom] += 1
        logger.info(f"  Val human domain distribution: {dict(dom_counts)}")

    return human_train, human_val, ai_train_by_model, ai_val_by_model


# ADAL v5: module-level val metadata (populated by load_raid_multigenerator)
# Used by the validation loop for per-domain / per-attack reporting.
_v5_val_human_meta:  List[Tuple[str, str]]              = []
_v5_val_ai_meta:     Dict[str, List[Tuple[str, str]]]   = {}
_v5_heldout_in_val:  List[str]                          = []


# ══════════════════════════════════════════════════════════════════════════════
#  PAN CLEF JSONL Data Loader (ADAL v4)
# ══════════════════════════════════════════════════════════════════════════════
def _load_one_jsonl(path: str) -> Tuple[List[str], Dict[str, List[str]]]:
    """
    Read a JSONL file where each line is { text, label, model, ... }.
    Returns (human_texts, ai_texts_by_model). label=0 → human, label=1 → AI.
    Skips texts shorter than 20 words to match the RAID loader's threshold.
    """
    human_texts: List[str]            = []
...
def load_panclef_jsonl_dataset() -> Tuple[
    ...
    """
    Load PAN CLEF train.jsonl + val.jsonl. Pre-split files (no internal split).
    """
    ...
    human_train, ai_train_by_model = _load_one_jsonl(TRAIN_JSONL)
    human_val,   ai_val_by_model   = _load_one_jsonl(VAL_JSONL)

    # Train/val leakage check (carry-over from v2 debugging)
    ...
    if overlap_ai > 0 or overlap_hum > 0:
        logger.warning(
            f"  ⚠ Train/val leakage: {overlap_ai} AI texts + {overlap_hum} "
            f"human texts in BOTH splits. This will inflate val metrics."
        )
    else:
    ...
    if only_train:
        logger.warning(f"  Generators in train only: {only_train}")
    if only_val:
        logger.warning(f"  Generators in val only: {only_val}")
...
def load_dataset() -> Tuple[
    ...
    """
    ADAL v4: Unified dataset router. Dispatches to RAID or PAN CLEF based on
    the global DATASET variable (settable via --dataset CLI flag).
    """
    if DATASET == "raid":
        if not _RAID_AVAILABLE:
            raise RuntimeError(
                "DATASET='raid' but raid-bench is not installed.\n"
                "Install with: pip install raid-bench\n"
                "Or run with --dataset panclef instead."
            )
        return load_raid_multigenerator()
    elif DATASET == "panclef":
        return load_panclef_jsonl_dataset()
    else:
        raise ValueError(f"Unknown DATASET={DATASET!r}. Use 'raid' or 'panclef'.")
...
class GeneratorScheduler:
    def __init__(self, generators: List[str], strategy: str):
        ...
        # ADAL v4: hard-negative sampling state
        # Per-generator weights are computed from val TPR@1%FPR. Initially
        # uniform; updated each validation cycle by update_hard_neg_weights().
        self._hard_neg_weights: Dict[str, float] = {g: 1.0 for g in generators}
    ...
    def next(self, outer_step: int) -> List[str]:
        ...
        if self.strategy == "hard_negative":
            # ADAL v4: still return ALL generators (so the detector sees full
            # diversity each step), but the per-generator quotas in
            # sample_from_generators() respect _hard_neg_weights to up-weight
            # the worst performers. Use uniform behavior during warmup.
            if outer_step < HARD_NEG_WARMUP_STEPS:
                return list(self.generators)
            return list(self.generators)
    ...
    def update_hard_neg_weights(self, tpr_by_generator: Dict[str, float]) -> None:
        """
        ADAL v4: Pangram-style hard-negative weighting. Generators with low
        val TPR@1%FPR get exponentially more sampling probability.

        weight_g = (1 - tpr_g) ** HARD_NEG_WEIGHT_POWER + HARD_NEG_BASE_WEIGHT

        - Worst generator (tpr=0.81): weight ~= 0.19^2 + 0.1 = 0.136
        - Best generator (tpr=0.99):  weight ~= 0.01^2 + 0.1 = 0.100
        - Ratio: 1.36x more samples to the worst — modest but consistent shift.
        """
        new_weights: Dict[str, float] = {}
        for g in self.generators:
            tpr = tpr_by_generator.get(g, 0.5)  # default mid-quality if missing
            tpr = max(0.0, min(1.0, tpr))
            w   = (1.0 - tpr) ** HARD_NEG_WEIGHT_POWER + HARD_NEG_BASE_WEIGHT
            new_weights[g] = w
        # Normalize so total weight equals number of generators (preserves
        # average sample budget across generators).
        total = sum(new_weights.values())
        n     = len(self.generators)
        for g in new_weights:
            new_weights[g] *= n / total

        self._hard_neg_weights = new_weights
        ranked = sorted(new_weights.items(), key=lambda kv: -kv[1])
        logger.info(
            "  [HardNeg] weights (high=more samples): "
            + "  ".join(f"{g}={w:.3f}" for g, w in ranked[:5])
            + f"  ...  (range: {min(new_weights.values()):.3f}–{max(new_weights.values()):.3f})"
        )

    def get_hard_neg_weights(self) -> Dict[str, float]:
        return dict(self._hard_neg_weights)
...
def sample_from_generators(
    ...
    n: int,
    hard_neg_weights:   Optional[Dict[str, float]] = None,
) -> Tuple[List[str], List[str]]:
    """
    Sample `n` AI texts from the active generators. With hard_neg_weights,
    quotas per generator are weighted by inverse TPR (ADAL v4).
    """
    ...
    # ADAL v4: hard-negative quota allocation
    if hard_neg_weights is not None:
        # Allocate quota = base_size * weight, ensuring every generator gets ≥1
        weights = [hard_neg_weights.get(g, 1.0) for g in active_generators]
        wsum    = sum(weights)
        sizes   = [max(1, round(n * w / wsum)) for w in weights]
    else:
        sizes = [len(ai_train_by_model[g]) for g in active_generators]
        total = sum(sizes)
        sizes = [max(1, round(n * s / total)) for s in sizes]

    xm_texts: List[str] = []
    xm_srcs:  List[str] = []
    for g, quota in zip(active_generators, sizes):
        pool = ai_train_by_model[g]
        if not pool:
            continue
        draw = random.choices(pool, k=min(quota, len(pool) * 10))
...
class Detector(nn.Module):
    def __init__(self):
        ...
        self.model.to(DEVICE)
        # ADAL v4: auxiliary multiclass generator-attribution head.
        # Input  = pooled hidden state [hidden_size]
        # Output = N+1 logits (one per generator + 1 for "human").
        # The mapping {generator_name: class_id} is set at training time via
        # set_aux_head(num_classes). Until then, aux_head is None.
        self.aux_head: Optional[nn.Module] = None
        self.aux_class_map: Dict[str, int] = {}

    def set_aux_head(self, generators: List[str]) -> None:
        """ADAL v4: build the auxiliary head once we know the generator list.
        Class 0 = "human"; classes 1..N = generators (sorted alphabetically)."""
        if not AUX_HEAD_ENABLED:
            return
        sorted_gens = sorted(generators)
        self.aux_class_map = {"human": 0}
        for i, g in enumerate(sorted_gens):
            self.aux_class_map[g] = i + 1
        num_classes = len(self.aux_class_map)
        hidden_size = self.model.config.hidden_size
        self.aux_head = nn.Sequential(
            nn.Linear(hidden_size, hidden_size),
            nn.GELU(),
            nn.Dropout(AUX_HEAD_DROPOUT),
            nn.Linear(hidden_size, num_classes),
        ).to(DEVICE)
        logger.info(
            f"  Auxiliary head built: {num_classes} classes "
            f"(human + {len(sorted_gens)} generators)"
        )
    ...
    def _encode_pooled(
        ...
        """
        Forward through RoBERTa, returning (binary_logits, pooled_hidden).
        pooled_hidden is the [CLS] embedding after the classifier dense layer
        — same input the binary head uses, so the auxiliary head shares
        encoder features without disturbing them.
        """
        ...
        pooled  = seq_out[:, 0, :]                    # [CLS] token
        # Apply the classifier's pre-activation dense + tanh to match how
        # RobertaClassificationHead processes it (without the final out_proj).
        cls_head = self.model.classifier
        x = cls_head.dropout(pooled)
        x = cls_head.dense(x)
        x = torch.tanh(x)
        x = cls_head.dropout(x)
        binary_logits = cls_head.out_proj(x)          # [B, 2]
        return binary_logits, x                       # x is the H-dim pooled feature

    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        """Default forward: returns P(human). Used by PPO reward computation."""
    ...
    def forward_with_aux(
        ...
        """
        ADAL v4: forward returning BOTH binary P(human) AND multiclass logits.
        Used during detector training when the auxiliary head is enabled.

        Returns (p_human [B], aux_logits [B, num_classes] or None).
        """
        if self.aux_head is None or not AUX_HEAD_ENABLED:
            p = self.forward(input_ids, attention_mask)
            return p, None
        binary_logits, pooled = self._encode_pooled(input_ids, attention_mask)
        p_human    = torch.softmax(binary_logits, dim=-1)[:, 1]
        aux_logits = self.aux_head(pooled)
        return p_human, aux_logits
    ...
    def save(self, path: str) -> None:
        ...
        self.model.save_pretrained(path)
        self.tokenizer.save_pretrained(path)
        # ADAL v4: persist auxiliary head if present (multiclass generator-id)
        if hasattr(self, "aux_head") and self.aux_head is not None:
            torch.save(self.aux_head.state_dict(), os.path.join(path, "aux_head.pt"))
        logger.info(f"Detector saved → {path}")

    def load(self, path: str) -> None:
        """ADAL v4: reload a saved detector in-place. Used by the post-training
        isotonic calibration step which instantiates a fresh Detector and
        loads the best checkpoint into it."""
        logger.info(f"Loading detector weights from {path} …")
        # Reload tokenizer + base model
        self.tokenizer = RobertaTokenizer.from_pretrained(path)
        self.model     = RobertaForSequenceClassification.from_pretrained(path).to(DEVICE)
        # Reload aux_head if it was saved
        aux_path = os.path.join(path, "aux_head.pt")
        if hasattr(self, "aux_head") and self.aux_head is not None and os.path.exists(aux_path):
            self.aux_head.load_state_dict(torch.load(aux_path, map_location=DEVICE))
            self.aux_head.to(DEVICE)
            logger.info(f"  Auxiliary head reloaded from {aux_path}")
...
def detector_multievasion_loss_and_backward(
    ...
    # ── ADAL v4: per-text generator labels for auxiliary multiclass head ───
    # Aligned with xm_texts (clean AI) — strings naming the source generator.
    # Used only if detector.aux_head exists and AUX_HEAD_ENABLED=True.
    xm_sources:    Optional[List[str]] = None,
) -> Dict[str, float]:
    """
    ...
    ADAL v4 adds the `xm_sources` argument and an auxiliary multiclass head.

    Streams handled:
      1. xh_texts           → target=HUMAN, weight=1.0          (clean humans)
      2. xm_texts           → target=AI,    weight=1.0          (clean AI)
      3. xp_ppo_texts       → target=AI,    weight=ATTACK_LOSS_WEIGHTS["t5"] (Gσ)
      4. xh_humanized_texts → target=HUMAN, weight=HUMANIZER_LOSS_WEIGHT     (Gψ)
      plus N deterministic attack groups (all target=AI).

    ADAL v4 auxiliary head (active when detector.aux_head is not None):
      - xh_texts contributes label=0 ("human") to the multiclass head
      - xm_texts contributes label=class_id_for(source_generator)
      - xh_humanized_texts contributes label=0 ("human")
      - xp_ppo_texts and deterministic attacks are excluded from aux loss
        (they're modified — generator label is no longer reliable)
    ...
    use_aux = (
        AUX_HEAD_ENABLED
        and detector.aux_head is not None
        and len(detector.aux_class_map) > 1
    )
    ...
    def process_group(
        ...
        weight:   float,
        tag:      str,
        aux_labels: Optional[List[int]] = None,   # ADAL v4: aux class ids
    ) -> Tuple[float, float]:
        """
        Forward + immediate backward for one text group in micro-batches.
        Returns (mean_binary_loss, mean_aux_loss). aux loss is 0 if disabled.

        ADAL v4: when aux_labels is provided AND use_aux=True, we use
        forward_with_aux() to get both binary P(human) and multiclass logits,
        then add AUX_HEAD_LOSS_WEIGHT * cross_entropy as a side loss.
        """
        if not texts:
            return 0.0, 0.0
        # Default aux labels: HUMAN=class 0 for human groups; if no labels
        # passed for AI groups, skip aux loss for those.
        if use_aux and aux_labels is None and is_human:
            aux_labels = [0] * len(texts)
        ...
        for i in range(0, len(texts), micro_batch):
            ...
            if not mb:
                continue
            ids, mask = encode(mb)
            if use_aux and aux_labels is not None:
                p, aux_logits = detector.forward_with_aux(ids, mask)
                mb_aux_labels = aux_labels[i : i + micro_batch]
                aux_target = torch.tensor(
                    mb_aux_labels, dtype=torch.long, device=p.device
                )
                aux_loss = F.cross_entropy(aux_logits, aux_target)
            else:
                p        = detector(ids, mask)
                aux_loss = torch.tensor(0.0, device=p.device)

            loss = _binary_smooth_loss(p, is_human, ls_alpha)
            scaled_main = weight * loss if is_human else lambda_coeff * weight * loss
            scaled_aux  = AUX_HEAD_LOSS_WEIGHT * weight * aux_loss
            (scaled_main + scaled_aux).backward()

            total_loss_val += loss.item()
            total_aux_val  += float(aux_loss.item()) if use_aux else 0.0
            n_micro        += 1
            del ids, mask, p, loss, scaled_main, scaled_aux, aux_loss
            torch.cuda.empty_cache()
        denom = max(n_micro, 1)
        return total_loss_val / denom, total_aux_val / denom
    ...
    # ADAL v4: build aux labels for the AI-clean stream (one per xm_text)
    aux_labels_xm: Optional[List[int]] = None
    if use_aux and xm_sources is not None and xm_texts:
        aux_labels_xm = [
            detector.aux_class_map.get(g, 0)  # 0 = "human" fallback if unknown
            for g in xm_sources[: len(xm_texts)]
        ]
        # Pad if mismatched (shouldn't happen, but guard against it)
        while len(aux_labels_xm) < len(xm_texts):
            aux_labels_xm.append(0)

    # Group 1: Clean human texts
    main, aux = process_group(xh_texts, True, 1.0, "xh")
    component_losses["L_human"] = main
    if use_aux:
        component_losses["L_aux_xh"] = aux

    # Group 2: Clean AI texts
    main, aux = process_group(xm_texts, False, 1.0, "xm", aux_labels=aux_labels_xm)
    component_losses["L_xm_original"] = main
    if use_aux:
        component_losses["L_aux_xm"] = aux

    # Group 3: T5 PPO paraphrase (Gσ output — AI→human-ish; aux skipped)
    if xp_ppo_texts:
        w = ATTACK_LOSS_WEIGHTS.get("t5_paraphrase", 1.0)
        main, _ = process_group(xp_ppo_texts, False, w, "t5")
        component_losses["L_t5_para"] = main

    # Group 4 (ADAL v3): Humanizer Gψ output — still human, train as human
    if xh_humanized_texts:
        main, aux = process_group(
            xh_humanized_texts, True, HUMANIZER_LOSS_WEIGHT, "hum",
        )
        component_losses["L_xh_humanized"] = main
        if use_aux:
            component_losses["L_aux_hum"] = aux

    # Groups 5+: Deterministic attacks — one at a time (aux skipped, text modified)
    for atk_name, atk_texts in xp_det_by_atk.items():
        if not atk_texts:
            continue
        w = ATTACK_LOSS_WEIGHTS.get(atk_name, 0.5)
        main, _ = process_group(atk_texts, False, w, atk_name)
        component_losses[f"L_{atk_name}"] = main
...
def evaluate_per_domain(
    detector:        Detector,
    human_val:       List[str],
    human_val_meta:  List[Tuple[str, str]],
    ai_val_by_model: Dict[str, List[str]],
    ai_val_meta:     Dict[str, List[Tuple[str, str]]],
    outer_step:      int,
) -> Dict[str, Dict[str, float]]:
    """
    ADAL v5: Compute AUROC and TPR@1%FPR sliced by DOMAIN.
    Uses metadata captured by load_raid_multigenerator into _v5_val_*_meta.

    Returns dict: {domain: {auroc, tpr_at_1fpr, n_human, n_ai}}
    """
    if not human_val_meta or not ai_val_meta:
        return {}

    # Group human texts by domain
    human_by_domain: Dict[str, List[str]] = defaultdict(list)
    for text, (dom, atk) in zip(human_val, human_val_meta):
        human_by_domain[dom].append(text)

    # Group AI texts by domain (across all generators)
    ai_by_domain: Dict[str, List[str]] = defaultdict(list)
    for gen, texts in ai_val_by_model.items():
        metas = ai_val_meta.get(gen, [])
        for text, meta in zip(texts, metas):
            dom = meta[0]
            ai_by_domain[dom].append(text)

    all_domains = sorted(set(human_by_domain.keys()) | set(ai_by_domain.keys()))
    results: Dict[str, Dict[str, float]] = {}

    for dom in all_domains:
        humans = human_by_domain.get(dom, [])
        ais    = ai_by_domain.get(dom, [])
        if not humans or not ais:
            continue   # need both classes for AUROC
        h_probs = detector.predict_human_prob(humans)
        a_probs = detector.predict_human_prob(ais)
        n = min(len(h_probs), len(a_probs))
        scores = np.concatenate([h_probs[:n], a_probs[:n]])
        labels = np.concatenate([np.ones(n), np.zeros(n)])
        try:
            auroc_val = float(roc_auc_score(labels, scores))
        except ValueError:
            auroc_val = 0.5
        # TPR@1%FPR using all (not just min n)
        tpr_labels = np.concatenate([np.zeros(len(h_probs)), np.ones(len(a_probs))])
        tpr_scores = np.concatenate([1.0 - h_probs, 1.0 - a_probs])
        tpr = tpr_at_fpr(tpr_labels, tpr_scores, target_fpr=TARGET_FPR)
        results[dom] = {
            "auroc":        auroc_val,
            "tpr_at_1fpr":  tpr,
...
def evaluate_per_attack_from_data(
    detector:        Detector,
    human_val:       List[str],
    ai_val_by_model: Dict[str, List[str]],
    ai_val_meta:     Dict[str, List[Tuple[str, str]]],
    ...
    """
    ADAL v5: Compute AUROC sliced by ATTACK using REAL adversarial data
    (not just synthetic attacks applied at validation time). This complements
    evaluate_auroc_per_attack which uses our deterministic attack pool.
    """
    if not ai_val_meta:
        return {}

    # Group AI texts by attack name
    ai_by_attack: Dict[str, List[str]] = defaultdict(list)
    for gen, texts in ai_val_by_model.items():
        metas = ai_val_meta.get(gen, [])
        for text, meta in zip(texts, metas):
            atk = meta[1]
            ai_by_attack[atk].append(text)

    if not ai_by_attack:
        return {}

    h_probs = detector.predict_human_prob(human_val)
    results: Dict[str, Dict[str, float]] = {}

    for atk, ais in sorted(ai_by_attack.items()):
        if not ais:
            continue
        a_probs = detector.predict_human_prob(ais)
        n = min(len(h_probs), len(a_probs))
...
def log_v5_diagnostics(
    domain_results:     Dict[str, Dict[str, float]],
    attack_data_results: Dict[str, Dict[str, float]],
    heldout_generators:  List[str],
    gen_aurocs:         Dict[str, float],
    gen_tprs:           Dict[str, float],
) -> None:
    """ADAL v5: pretty-print per-domain and real-attack diagnostics."""
    if domain_results:
        logger.info("  ┌─ Per-domain val metrics (REAL data, not synthesised) ──┐")
        for dom in sorted(domain_results.keys()):
            r = domain_results[dom]
            logger.info(
                f"  │  {dom:18s} AUROC={r['auroc']:.4f}  "
                f"TPR@1%FPR={r['tpr_at_1fpr']:.4f}  "
                f"(h={r['n_human']:>4} a={r['n_ai']:>5})"
            )
        logger.info("  └─────────────────────────────────────────────────────────┘")

    if attack_data_results:
        logger.info("  ┌─ Per-attack val (REAL RAID adversarial data) ──────────┐")
        for atk in sorted(attack_data_results.keys()):
            r = attack_data_results[atk]
            warn = " ⚠" if r["auroc"] < 0.9 else ""
            logger.info(
                f"  │  {atk:24s} AUROC={r['auroc']:.4f}  "
                f"TPR@1%FPR={r['tpr_at_1fpr']:.4f}  (n={r['n_ai']:>5}){warn}"
            )
        logger.info("  └─────────────────────────────────────────────────────────┘")

    if heldout_generators:
        logger.info("  ┌─ Held-out generators (NEVER seen during training) ─────┐")
        for gen in heldout_generators:
            a = gen_aurocs.get(gen, float("nan"))
            t = gen_tprs.get(gen, float("nan"))
            warn = " ⚠ MEMORIZATION" if a > 0.99 and t > 0.95 else ""
            logger.info(
                f"  │  {gen:24s} AUROC={a:.4f}  TPR@1%FPR={t:.4f}{warn}"
            )
        logger.info("  └─────────────────────────────────────────────────────────┘")
...
def train_radar_multievasion():
    ...
    logger.info("═" * 72)
    logger.info("  ADAL v5 — Honest Generalization Tests (RoBERTa-large)")
    logger.info("  (v4 base + RAID-extra · held-out generators · adversarial val)")
    logger.info("  v4 additions (carried over):")
    logger.info(f"    • Hard-negative generator sampling (strategy={GENERATOR_SAMPLING_STRATEGY!r})")
    logger.info(f"    • Multiclass auxiliary head (enabled={AUX_HEAD_ENABLED}  "
                f"weight={AUX_HEAD_LOSS_WEIGHT})")
    logger.info(f"    • Worst-generator best-ckpt selection "
                f"(enabled={USE_WORST_GENERATOR_FOR_BEST})")
    logger.info("  v5 additions:")
    logger.info(f"    1. RAID + RAID-extra            : USE_RAID_EXTRA={USE_RAID_EXTRA}")
    logger.info(f"    2. Held-out generators in val   : {HELDOUT_GENERATORS or '(none)'}")
    logger.info(f"    3. Adversarial val split        : USE_ADVERSARIAL_VAL={USE_ADVERSARIAL_VAL}  "
                f"(cap={ADVERSARIAL_VAL_PER_ATTACK_CAP}/attack)")
    logger.info(f"    4. Per-domain reporting         : REPORT_PER_DOMAIN_METRICS={REPORT_PER_DOMAIN_METRICS}")
    logger.info(f"  Dataset: {DATASET!r}  "
    ...
    logger.info(f"  Target metric: TPR@{int(TARGET_FPR*100)}%FPR  "
                f"(use_tpr_for_best={USE_TPR_AT_FPR_FOR_BEST}, "
                f"use_worst_gen_for_best={USE_WORST_GENERATOR_FOR_BEST})")
    ...
    logger.info(f"  Hard-negative: power={HARD_NEG_WEIGHT_POWER}  "
                f"base={HARD_NEG_BASE_WEIGHT}  warmup={HARD_NEG_WARMUP_STEPS}")
    ...
    # ── 1. Load data ───────────────────────────────────────────────────────
    human_train, human_val, ai_train_by_model, ai_val_by_model = load_dataset()
    generators = sorted(ai_train_by_model.keys())
    if not generators:
        raise RuntimeError(
            f"No AI generator data found in DATASET={DATASET!r}. "
            "Check RAID filters or PAN CLEF JSONL paths."
        )
    ...
    paraphraser  = Paraphraser()
    detector     = Detector()
    # ADAL v4: build the auxiliary multiclass head now that we know the
    # generator list. set_aux_head is a no-op if AUX_HEAD_ENABLED=False.
    detector.set_aux_head(generators)
    ...
    best_worst_gen_tpr = 0.0 # ADAL v4: max(min(per-gen TPR)) tracker
    ...
    for outer_step in range(1, MAX_OUTER_STEPS + 1):
        ...
        # ADAL v4: hard-negative quotas — if scheduler is in hard_negative mode
        # and warmup is past, use the inverse-TPR weights to allocate per-gen
        # samples. Other strategies pass None → uniform/proportional quotas.
        hn_weights = (
            scheduler.get_hard_neg_weights()
            if (GENERATOR_SAMPLING_STRATEGY == "hard_negative"
                and outer_step >= HARD_NEG_WARMUP_STEPS)
            else None
        )
        batch_xm, xm_sources = sample_from_generators(
            ai_train_by_model, active_gens, n_samples,
            hard_neg_weights=hn_weights,
        )
        ...
        if not skip_detector:
            ...
            for i in range(0, len(idx_list), DETECTOR_BATCH):
                ...
                # ADAL v4: xm_sources_batch needed for auxiliary multiclass head.
                xm_sources_batch = [buffer.source_models[j] for j in b]
                loss_details = detector_multievasion_loss_and_backward(
                    ...
                    xm_sources=xm_sources_batch,        # ADAL v4: aux head labels
                )
                nn.utils.clip_grad_norm_(detector.parameters(), GRAD_CLIP)
                # ADAL v4: also clip auxiliary head gradients
                if detector.aux_head is not None:
                    nn.utils.clip_grad_norm_(detector.aux_head.parameters(), GRAD_CLIP)
                d_opt.step()
                d_sched.step()
        ...
        if outer_step % VALIDATE_EVERY == 0 or outer_step == 1:
            ...
            # ── ADAL v5: Per-domain + real-attack + held-out diagnostics ──
            if DATASET == "raid" and (REPORT_PER_DOMAIN_METRICS
                                      or USE_ADVERSARIAL_VAL
                                      or _v5_heldout_in_val):
                domain_results = evaluate_per_domain(
                    detector, human_val, _v5_val_human_meta,
                    ai_val_by_model, _v5_val_ai_meta, outer_step,
                ) if REPORT_PER_DOMAIN_METRICS else {}
                attack_data_results = evaluate_per_attack_from_data(
                    detector, human_val, ai_val_by_model,
                    _v5_val_ai_meta, outer_step,
                ) if USE_ADVERSARIAL_VAL else {}
                log_v5_diagnostics(
                    domain_results, attack_data_results,
                    _v5_heldout_in_val, gen_aurocs, gen_tprs,
                )
            ...
            # ADAL v4: refresh hard-negative weights from the latest val TPRs
            if GENERATOR_SAMPLING_STRATEGY == "hard_negative":
                tpr_only = {k: v for k, v in gen_tprs.items() if k != "MACRO_AVG"}
                if tpr_only:
                    scheduler.update_hard_neg_weights(tpr_only)

            # ── ADAL v4: Compute worst-generator metric for selection ──────
            # Even with global TPR@1%FPR climbing, the WORST generator may be
            # falling. Track min(per-gen TPR@1%FPR) explicitly.
            tpr_only = {k: v for k, v in gen_tprs.items() if k != "MACRO_AVG"}
            worst_gen_tpr = float(min(tpr_only.values())) if tpr_only else 0.0
            worst_gen_name = (
                min(tpr_only.items(), key=lambda kv: kv[1])[0]
                if tpr_only else "n/a"
            )
            logger.info(
                f"  [Worst-gen] {worst_gen_name} TPR@1%FPR={worst_gen_tpr:.4f}"
            )

            # ── ADAL v2/v4: Best checkpoint selection ──────────────────────
            # Priority order:
            #   v4: USE_WORST_GENERATOR_FOR_BEST → max(min(per-gen TPR))
            #   v2: USE_TPR_AT_FPR_FOR_BEST     → max(global TPR@1%FPR)
            #   else                            → max(macro AUROC)
            if USE_WORST_GENERATOR_FOR_BEST:
                primary  = worst_gen_tpr
                best_pri = best_worst_gen_tpr
                metric_name = "worst-gen TPR@1%FPR"
            elif USE_TPR_AT_FPR_FOR_BEST:
            ...
            if is_new_best:
                ...
                best_auroc         = macro
                best_worst_gen_tpr = worst_gen_tpr   # ADAL v4
                ...
                logger.info(
                    ...
                    f"(AUROC={macro:.4f}, TPR@1%FPR={global_tpr_at_1fpr:.4f}, "
                    f"worst-gen={worst_gen_tpr:.4f} [{worst_gen_name}]) — saved.\n"
                )
                ...
                logger.info(
                    ...
                    f"worst-gen={worst_gen_tpr:.4f} [{worst_gen_name}]\n"
                )
    ...
    if FIT_ISOTONIC_CALIBRATOR:
        ...
        try:
            ...
            best_detector = Detector()   # ADAL v4: takes no args; uses globals
...
if __name__ == "__main__":
    # ADAL v4: minimal CLI for dataset selection. Other settings are still
    # edited at the top of this file (DETECTOR_LR, MAX_OUTER_STEPS, etc.).
    parser = argparse.ArgumentParser(
        description="ADAL v4 — RoBERTa-large + per-generator weakness mitigation",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--dataset", type=str, default=None, choices=["raid", "panclef"],
        help="Override DATASET. raid → RAID benchmark via raid-bench. "
             "panclef → JSONL files at TRAIN_JSONL / VAL_JSONL.",
    )
    parser.add_argument(
        "--train-jsonl", type=str, default=None,
        help="Override TRAIN_JSONL when --dataset panclef.",
    )
    parser.add_argument(
        "--val-jsonl", type=str, default=None,
        help="Override VAL_JSONL when --dataset panclef.",
    )
    parser.add_argument(
        "--output-dir", type=str, default=None,
        help="Override OUTPUT_DIR (where checkpoints + logs are saved).",
    )
    parser.add_argument(
        "--full-data", action="store_true",
        help="Use the entire dataset (NUM_HUMAN_SAMPLES=None, "
             "SAMPLES_PER_GENERATOR=None). Recommended for final runs.",
    )
    ...
    if args.output_dir is not None:
        ...
        DETECTOR_SAVE_PATH    = os.path.join(OUTPUT_DIR, "best_detector")
        PARAPHRASER_SAVE_PATH = os.path.join(OUTPUT_DIR, "best_paraphraser")
        HUMANIZER_SAVE_PATH   = os.path.join(OUTPUT_DIR, "best_humanizer")
        LOG_FILE              = os.path.join(OUTPUT_DIR, "training.log")
        AUROC_LOG_FILE        = os.path.join(OUTPUT_DIR, "per_generator_auroc.tsv")
        ATTACK_AUROC_LOG      = os.path.join(OUTPUT_DIR, "per_attack_auroc.tsv")
        os.makedirs(OUTPUT_DIR, exist_ok=True)
    if args.full_data:
        NUM_HUMAN_SAMPLES     = None
        SAMPLES_PER_GENERATOR = None
