"""
ADAL v4 — Per-Generator Weakness Mitigation (RoBERTa-large)
============================================================
Base    : "RADAR: Robust AI-Text Detection via Adversarial Learning"
          Hu et al., NeurIPS 2023 — https://arxiv.org/pdf/2307.03838
Datasets: RAID (Dugan et al., ACL 2024)  AND  PAN CLEF 2026 (custom JSONL)
          Switch via --dataset {raid|panclef} CLI flag.

v4 extensions over v3 (bidirectional adversarial game):
  • Hard-negative generator sampling — dynamically up-weight worst generators
    based on running val TPR@1%FPR (Pangram-style active learning, 2025)
  • Multiclass auxiliary head — generator-attribution side task that forces
    encoder to learn generator-discriminative features (Leidos COLING 2025)
  • Worst-generator best-checkpoint selection — max(min(per-gen TPR@1%FPR))
    instead of average; explicitly optimizes for the weakest link

Targets per-generator weakness on RAID (v3 best ckpt):
  - mpt: 0.808, cohere: 0.883, cohere-chat: 0.891
without requiring a backbone change (RoBERTa-large stays).
...
NUM_HUMAN_SAMPLES     = 10000  # RAID default; set None for full PAN CLEF
SAMPLES_PER_GENERATOR = 1000   # RAID default; set None for full PAN CLEF
...
# ── Output paths ──────────────────────────────────────────────────────────────
OUTPUT_DIR            = "./adal_v4_balanced_raid"
...
HF_TOKEN              = "Shushant"        # ← paste your HF write token
HF_USERNAME           = "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"   # ← MUST match exactly: huggingface.co/settings/profile
...
HF_DETECTOR_REPO      = "adal-v4-raidbalanced-detector"     # ← repo name for the detector
HF_PARAPHRASER_REPO   = "adal-v4-raidbalanced-paraphraser"       # ← repo name for the paraphraser
...
def load_raid_multigenerator() -> Tuple[
    ...
    """
    Load RAID train split, deduplicate humans, and class-balance AI to match
    the human count, then split 90/10 train/val.
    ...
    ADAL v4 strategy (CLASS-BALANCED):
      1. Load RAID train split (attack="none" only — clean AI + human texts).
      2. Deduplicate humans by source_id (each unique document appears 11×4=44
         times in raw RAID; we want one row per unique document).
      3. ADAL v4 — Class-balance: cap AI to max(SAMPLES_PER_GENERATOR, balanced)
         where balanced = len(humans) // num_generators. This makes total AI
         ≈ total humans, eliminating the 35× class imbalance that biased
         training in earlier versions.
      4. Split each list (human + per-generator AI) 90/10 by VAL_SPLIT_RATIO
    ...
    # ── Stage 1: parse with NO per-gen cap (we'll cap after we know human count)
    # We still respect NUM_HUMAN_SAMPLES if explicitly set; SAMPLES_PER_GENERATOR
    # is overridden to None here so we can compute the balanced quota below.
    human_all, ai_all_by_model = _parse_raid_df(
        train_df, "full_train", NUM_HUMAN_SAMPLES, n_ai_per_gen=None,
        attack_filter=RAID_FILTER_ATTACK,
    )

    n_humans = len(human_all)
    n_gens   = max(len(ai_all_by_model), 1)

    # ── ADAL v4: Stage 2 — class-balance AI to humans ─────────────────────
    # Goal: total AI ≈ total humans. Per-generator quota = n_humans / n_gens.
    # If user set SAMPLES_PER_GENERATOR explicitly, it acts as an UPPER bound.
    balanced_quota = max(1, n_humans // n_gens)
    if SAMPLES_PER_GENERATOR is not None:
        ...
        logger.info(
            f"  Class-balanced quota: {n_humans} humans / {n_gens} generators "
            f"= {per_gen_quota} AI texts/gen "
            f"(target: total AI ≈ total humans)"
        )

    ai_balanced_by_model: Dict[str, List[str]] = {}
    for gen, texts in ai_all_by_model.items():
        # Truncate each generator to per_gen_quota (texts are already shuffled
        # inside _parse_raid_df, so this is a uniform random subsample).
        ai_balanced_by_model[gen] = texts[:per_gen_quota]

    total_ai_balanced = sum(len(v) for v in ai_balanced_by_model.values())
    logger.info(
        f"  After balancing: human={n_humans:,}  AI={total_ai_balanced:,}  "
        f"(ratio AI/human = {total_ai_balanced/max(n_humans,1):.2f}×)"
    )

    # ── Stage 3: Split 90/10 train/val with fixed seed ────────────────────
    ...
    for gen, texts in ai_balanced_by_model.items():
...
def train_radar_multievasion():
    ...
    logger.info("═" * 72)
    logger.info("  ADAL v4 — Per-Generator Weakness Mitigation (RoBERTa-large)")
    logger.info("  (v3 base + 11 attacks · bidirectional Gσ/Gψ · TPR@1%FPR)")
    logger.info("  v4 additions:")
    logger.info(f"    1. Hard-negative generator sampling (strategy={GENERATOR_SAMPLING_STRATEGY!r})")
    logger.info(f"    2. Multiclass auxiliary head (enabled={AUX_HEAD_ENABLED}  "
                f"weight={AUX_HEAD_LOSS_WEIGHT})")
    logger.info(f"    3. Worst-generator best-ckpt selection "
