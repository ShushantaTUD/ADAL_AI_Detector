...
# Each line: {"id":..., "text":..., "model":..., "label": 0|1, "genre":...}
...
# Optional per-split caps. Set None to use all available samples.
GENERATORS_TO_USE:    Optional[List[str]] = None   # None = all generators
...
HF_TOKEN              = "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"        # ← paste your HF write token
...
HF_DETECTOR_REPO      = "ADAL-Detector-PANCLEF"     # ← repo name for the detector
HF_PARAPHRASER_REPO   = "ADAL_PAN-t5-paraphraser"       # ← repo name for the paraphraser
...
# ══════════════════════════════════════════════════════════════════════════════
#  Custom JSONL Data Loader
# ══════════════════════════════════════════════════════════════════════════════

def load_jsonl(path: str) -> Tuple[List[str], Dict[str, List[str]]]:
    """
    Load a JSONL file where each line is a JSON object with fields:
...
def load_jsonl_dataset() -> Tuple[
    List[str], List[str],
    Dict[str, List[str]], Dict[str, List[str]],
]:
    """
    Load TRAIN_JSONL and VAL_JSONL for ADAL adversarial training.
    Returns: human_train, human_val, ai_train_by_model, ai_val_by_model
    """
    logger.info("Loading custom JSONL dataset ...")
    ...
    only_train = set(ai_train_by_model) - set(ai_val_by_model)
    only_val   = set(ai_val_by_model)   - set(ai_train_by_model)
    if only_train:
    ...
    logger.info(f"  Train -> human={len(human_train):,}  AI={sum(len(v) for v in ai_train_by_model.values()):,}")
    logger.info(f"  Val   -> human={len(human_val):,}  AI={sum(len(v) for v in ai_val_by_model.values()):,}")
...
def evaluate_full_metrics(
    ...
    """
    Compute Precision, Recall, F1, and AUROC on the full validation set.

    Convention (matches detector training):
        P(human) high  → predict HUMAN  (negative class for AI detection)
        P(human) low   → predict AI     (positive class for AI detection)

    So for AI-detection metrics:
        True Positive  = AI text correctly predicted as AI  (score < threshold)
        True Negative  = human text correctly predicted as human (score >= threshold)

    Returns dict with keys:
        precision, recall, f1, auroc,
        per-generator auroc values,
        total_human, total_ai, tp, tn, fp, fn
    """
    all_ai_texts = [t for texts in ai_val_by_model.values() for t in texts]

    logger.info("  Running full validation evaluation ...")

    # Get probabilities
    human_probs = detector.predict_human_prob(human_val)     # high = human
    ai_probs    = detector.predict_human_prob(all_ai_texts)  # low  = AI

    # Build binary labels and scores for sklearn
    # label=1 means "AI-generated" (the positive detection class)
    # score  = 1 - P(human) = P(AI)
    y_true  = np.concatenate([np.zeros(len(human_probs)), np.ones(len(ai_probs))])
    p_ai    = np.concatenate([1 - human_probs,            1 - ai_probs          ])

    # Threshold predictions: p_ai >= threshold → predict AI (1)
    ...
    try:
        ...
        auroc     = float(roc_auc_score(y_true, p_ai))
    except ValueError:
        precision = recall = f1 = auroc = 0.0

    # Per-generator AUROC
    per_gen: Dict[str, float] = {}
    for gen, ai_texts in sorted(ai_val_by_model.items()):
        if not ai_texts:
            continue
        gen_ai_probs = detector.predict_human_prob(ai_texts)
        n = min(len(human_probs), len(gen_ai_probs))
        try:
            g_auroc = float(roc_auc_score(
                np.concatenate([np.zeros(n), np.ones(n)]),
                np.concatenate([1 - human_probs[:n], 1 - gen_ai_probs[:n]])
            ))
        except ValueError:
            g_auroc = 0.5
        per_gen[gen] = g_auroc

    macro_auroc = float(np.mean(list(per_gen.values()))) if per_gen else auroc

    # Pretty-print
    sep = "─" * 52
    logger.info(f"\n  ╔{'═'*52}╗")
    logger.info(f"  ║  FULL VALIDATION METRICS  (threshold={threshold:.2f})         ║")
    logger.info(f"  ╠{sep}╣")
    logger.info(f"  ║  Total human : {len(human_val):>6,}    Total AI : {len(all_ai_texts):>6,}        ║")
    logger.info(f"  ║  TP={tp:>5,}  TN={tn:>5,}  FP={fp:>5,}  FN={fn:>5,}         ║")
    logger.info(f"  ╠{sep}╣")
    ...
    logger.info(f"  ╠{sep}╣")
    logger.info(f"  ║  Per-generator AUROC:                                ║")
    for gen, val in sorted(per_gen.items()):
        logger.info(f"  ║    {gen:<30s}  {val:.4f}              ║")
    logger.info(f"  ║  Macro avg AUROC : {macro_auroc:.4f}                         ║")
    logger.info(f"  ╚{'═'*52}╝\n")
    ...
    metrics = {
        ...
        "total_human": len(human_val),
        "total_ai":    len(all_ai_texts),
        **{f"auroc_{g}": v for g, v in per_gen.items()},
    }
    ...
    if log_file:
        ...
        with open(log_file, "a", newline="") as csvf:
            writer = csv.DictWriter(csvf, fieldnames=list(metrics.keys()) + ["timestamp"])
            if write_hdr:
                writer.writeheader()
            writer.writerow({**metrics, "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")})
...
def train_radar_multievasion():
    ...
    logger.info("\n" + "═" * 72)
    logger.info("  FINAL EVALUATION ON VALIDATION SET (best checkpoint)")
    logger.info("═" * 72)
    # Load best detector checkpoint for final eval
    best_det = Detector()
    from transformers import RobertaForSequenceClassification, RobertaTokenizer
    best_det.tokenizer = RobertaTokenizer.from_pretrained(DETECTOR_SAVE_PATH)
    best_det.model     = RobertaForSequenceClassification.from_pretrained(
        DETECTOR_SAVE_PATH, num_labels=2
    )
    best_det.model.to(DEVICE)
    best_det.model.eval()

    metrics_log = os.path.join(OUTPUT_DIR, "validation_metrics.tsv")
    final_metrics = evaluate_full_metrics(
        best_det, human_val, ai_val_by_model,
        threshold=0.5,
        log_file=metrics_log,
    )
    logger.info(f"  Metrics saved → {metrics_log}")
    logger.info("═" * 72)
